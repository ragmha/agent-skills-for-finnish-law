...
test('every generated openai.yaml is structurally valid and matches its plugin mcp.json', () => {
  ...
  for (const entry of marketplace.plugins) {
    ...
    for (const skillDir of listDirs(join(ROOT, pluginRoot, 'skills'))) {
      ...
      assert.match(text, /^# Generated by scripts\/generate-adapters\.mjs/, `${where}: generointiotsake puuttuu`);
...
const EXPECTED_DEPENDENCIES = {
  'legal-core/skills/legal-research': ['oik-ai'],
  'contracts/skills/contract-review': ['adeu', 'oik-ai'],
  'ai-regulation/skills/ai-obligations': ['eu-ai-act'],
  'public-procurement/skills/tender-documents-and-bids': ['oik-ai'],
  'company-law/skills/shareholders-agreement': ['adeu', 'oik-ai'],
  // lähdevarmistuksen luovutuspisteet
  'insolvency/skills/debt-collection': ['oik-ai'],
  'legislative-drafting/skills/legislative-drafting-manual': ['oik-ai'],
  'ai-regulation/skills/ai-compliance': ['eu-ai-act', 'oik-ai'],
};
...
const FORBIDDEN_GENERATED_METADATA = [
  'legal-core/skills/practice-profile',
  'legal-core/skills/finnish-language',
];
...
test('workflows run the shared generated-files check', () => {
  ...
  assert.match(script, /generate-adapters\.mjs/);
